############################################################################
# WAVEFRONT ALGORITHM
#(x,y) indicates the position of the bot in the map, where x increases in the
# downward direction and y increases in the right direction.
############################################################################
try:
    import numpy
except:
...
class waveFrontPlanner:
    ...
    def __init__(self, slow=False, map_=floormap):
        self.__slow = slow
        self.__map = map_
        if str(type(map_)).find("numpy") != -1:
    ...
    ###########################################################################
# if prnt is False, we only get a list of tuples indicating the shortest path
# for the bot in the map in the form of (x,y)
    def run(self, prnt=False):
        ...
        if self._map[self._robot_x][self._robot_y]==self._goal:
